...
#                print ("Waiting for messages ... ")
                ...
                print (self.alias + ": " + e + " - Connection reset by peer - closing socket")
                self.sock.close()
                self.isAtive = False
                self.connect() # reconnect
                heartbeatFailCount = 0
...
exit = False
while not exit:
    msg = input("Enter 'DEVICE <alias> <host> [<port>(10000)]' or 'EXIT [<alias> ...]' or '<alias> <cmd>':\n")
    words = msg.split()
    if len(words) > 0 and words[0].upper() == "EXIT":
        if len(words) == 1: # stop all sockets and remove from list
            auxList = []
            for alias, sock in sockets.items():
                sock.stop()
